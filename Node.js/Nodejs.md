...
+ 我们在开发过程中使用一些简单好记的就可以了，例如 3000、5000 等没有什么含义
+ 可以同时开启多个服务，但是不同服务占用的端口号不能一样
+ `Content-Type`服务器响应的数据类型，不同的资源对应的`Content-Type`都不一样，图片不用加编码类型，文本类型的数据最好加上编码。防止中文解析乱码

> // 告诉浏览器你要发送的内容是什么编码  text/plain:普通文本  text/html:html格式的字符串
...
+ 参数一：当渲染以 `.html`结尾的文件的时候，使用 `art-template`模板引擎
...
+ Express 为 `Response` 相应对象提供了一个方法：`render`，`render`方法默认不可以使用的，但是如果配置了模板引擎就可以使用了 `response.renser('html模板名称',{模板数据})`
+ 第一个参数不能写路径，默认会去项目中的 views 目录查找该模板文件，Express 有一个约定：开发人员把所有视图文件都放到 views 目录中
+ 如果没有需要渲染的模板数据，第二个参数可以省略
...
```javascript
const student = require('./hooks/student.js')

// 调用获取学生数组的方法
  student.find((err,data) =>{
    if(err){
      return console.log('获取学生数组失败')
    }
    console.log(data)
  })
```
...
// 		第一个参数：传入一个大写名词单数字符串用来表示你的集合的名称
...
```javascript
User.deleteMany({ username: 'admin' },(err,ret) =>{
  if(ret.deletedCount === 0){
    console.log('删除失败')
  }else{
    console.log('删除成功，数量：',ret.deletedCount)
  }
...
```javascript
User.deleteOne({ username: 'admin' },(err,ret) =>{
  if(ret.deletedCount === 0){
    console.log('删除失败')
  }else{
    console.log('删除成功，数量：',ret.deletedCount)
...
```javascript
User.updateMany({ username: '李四' },(err,ret) =>{
...
```javascript
User.updateOne({ username: '李四' },{ password:'123456' },(err,ret) =>{
  if(ret.modifiedCount === 0){
    console.log('更新失败')
  }else{
    console.log('更新成功')
...
```javascript
// 更新	根据id来修改表数据
User.findByIdAndUpdate('61a1fcb31eebaf2fc088108c',{
  username: '李四'
},(err,ret) =>{
  if(err != null){
    console.log('更新失败')
...
public							公共静态资源
routes							如果业务比较大，把路由按业务分类存储到`routes`目录中
route.js    				把所有路由都放到这个文件
views								存储视图目录
```
